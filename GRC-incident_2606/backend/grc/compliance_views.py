...
def get_policies(request, framework_id):
    ...
    return Response([{'PolicyId': p.PolicyId, 'PolicyName': p.PolicyName} for p in policies])
 
@api_view(['GET'])
def get_subpolicies(request, policy_id):
    subpolicies = SubPolicy.objects.filter(PolicyId=policy_id, Status='Active')  # type: ignore
...
def get_all_users(request):
    ...
    try:
        ...
        users = User.objects.exclude(UserId=1).values('UserId', 'UserName')
...
def initialize_default_categories(request):
    """
    Initialize default category values if they don't exist
    """
    try:
        from .models import CategoryBusinessUnit
