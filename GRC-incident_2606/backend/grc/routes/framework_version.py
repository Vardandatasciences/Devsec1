...
def create_framework_version(request, framework_id):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # Get the latest version for this framework identifier
            ...
            # Create new framework based on original with updated data
            new_framework = Framework.objects.create(
                FrameworkName=framework_data.get('FrameworkName', original_framework.FrameworkName),
                FrameworkDescription=framework_data.get('FrameworkDescription', original_framework.FrameworkDescription),
                Category=framework_data.get('Category', original_framework.Category),
                ...
                DocURL=framework_data.get('DocURL', original_framework.DocURL),
                Identifier=framework_data.get('Identifier', original_framework.Identifier),
                CreatedByName=framework_data.get('CreatedByName', original_framework.CreatedByName),
                CreatedByDate=timezone.now().date(),
                Reviewer=reviewer_name or framework_data.get('Reviewer', original_framework.Reviewer),
                Status='Under Review',  # Always start as Under Review
                ActiveInactive='InActive',
                CurrentVersion=float(new_version)  # Set the CurrentVersion to the new version number
            )
                    ...
                    if original_policy_id:
                        try:
                            ...
                            new_policy = Policy.objects.create(
                                FrameworkId=new_framework,
                                PolicyName=policy_data.get('PolicyName', original_policy.PolicyName),
                                ...
                                PermanentTemporary='',
                                ActiveInactive='InActive',
                                Reviewer=policy_data.get('ReviewerName', policy_data.get('Reviewer', original_policy.Reviewer)),
                                CoverageRate=policy_data.get('CoverageRate', original_policy.CoverageRate),
                                CurrentVersion=str(float(new_version)),  # Set the CurrentVersion to match the framework version
                                ...
                                for subpolicy_data in policy_data.get('subpolicies', []):
                                    ...
                                    if original_subpolicy_id:
                                        try:
                                            ...
                                            # Create new subpolicy based on original with updated data
                                            SubPolicy.objects.create(
                                                PolicyId=new_policy,
                                                SubPolicyName=subpolicy_data.get('SubPolicyName', original_subpolicy.SubPolicyName),
                                                CreatedByName=subpolicy_data.get('CreatedByName', original_subpolicy.CreatedByName),
                                                CreatedByDate=timezone.now().date(),
                                                Identifier=subpolicy_data.get('Identifier', original_subpolicy.Identifier),
                                                Description=subpolicy_data.get('Description', original_subpolicy.Description),
                                                Status='Under Review',
                                                PermanentTemporary=subpolicy_data.get('PermanentTemporary', ''),
                                                Control=subpolicy_data.get('Control', original_subpolicy.Control)
                                            )
                                        ...
                                        print(f"Creating new subpolicy for existing policy {new_policy.PolicyName}")
                                        SubPolicy.objects.create(
                                            PolicyId=new_policy,
            ...
            if 'new_policies' in framework_data:
                for new_policy_data in framework_data.get('new_policies', []):
                    ...
                    new_policy = Policy.objects.create(
                        ...
                        CurrentVersion=str(float(new_version)),  # Set CurrentVersion to match the framework version
            ...
            if framework_reviewer_email:
                ...
                notification_data = {
                    ...
                    'email_type': 'gmail',
                    'template_data': [
                        new_framework.FrameworkName,
                        reviewer_name,
                        new_framework.CreatedByName,
                        str(new_version)
                    ]
                }
...
def activate_deactivate_framework_version(request, framework_id):
    """
    Activate or deactivate a specific framework version
    """
    try:
        # Get the framework
        framework = Framework.objects.get(FrameworkId=framework_id)
        ...
        new_active_status = 'Inactive' if framework.ActiveInactive == 'Active' else 'Active'
        framework.ActiveInactive = new_active_status
        ...
        return Response({
            "message": f"Framework version {new_active_status.lower()}d successfully",
            "FrameworkId": framework.FrameworkId,
            "ActiveInactive": framework.ActiveInactive,
            "Status": framework.Status
