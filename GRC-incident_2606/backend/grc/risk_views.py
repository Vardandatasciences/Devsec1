...
def get_reviewer_id(reviewer_name):
    ...
    try:
        ...
        from django.db import connection
        with connection.cursor() as cursor:
            cursor.execute("SELECT user_id FROM grc.user WHERE user_name = %s", [reviewer_name])
            row = cursor.fetchone()
            print(row,'-------------------------------------------')
            if row:
                return row[0]
...
def assign_reviewer(request):
    ...
    print(request.data,'-------------------------------------------')
    risk_id = request.data.get('risk_id')
    reviewer_id = request.data.get('reviewer_id')  # Use reviewer_id directly from request data
    ...
    try:
        ...
        if isinstance(reviewer_id, str) and not reviewer_id.isdigit():
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT user_id, user_name, email FROM grc.user WHERE user_name = %s", [reviewer_id])
                reviewer = cursor.fetchone()
                
                if not reviewer:
                    # Try to find by partial match
                    cursor.execute("SELECT user_id, user_name, email FROM grc.user WHERE user_name LIKE %s LIMIT 1", [f"%{reviewer_id}%"])
                    reviewer = cursor.fetchone()
        else:
            # It's already an ID or a string that can be converted to an ID
            with connection.cursor() as cursor:
                cursor.execute("SELECT user_id, user_name, email FROM grc.user WHERE user_id = %s", [reviewer_id])
                reviewer = cursor.fetchone()
        
        # Also get user info
        with connection.cursor() as cursor:
            cursor.execute("SELECT user_id, user_name, email FROM grc.user WHERE user_id = %s", [user_id])
            user = cursor.fetchone()
        ...
        reviewer_id_value = int(reviewer[0]) if reviewer[0] else None  # Use the actual user_id from the database
        print(f"Setting ReviewerId to {reviewer_id_value} (type: {type(reviewer_id_value)})")
        risk_instance.ReviewerId = reviewer_id_value
        risk_instance.ReviewerName = reviewer[1]  # reviewer_name
...
def get_users_for_dropdown(request):
    ...
    try:
        # Using raw SQL query to fetch from custom user table
        from django.db import connection
        with connection.cursor() as cursor:
            cursor.execute("SELECT user_id, user_name, email, department, designation FROM grc.user")
            columns = ['user_id', 'user_name', 'email', 'department', 'designation']
            users = [
                dict(zip(columns, row))
                for row in cursor.fetchall()
            ]
