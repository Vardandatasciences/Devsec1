...
def get_audit_pass_rate(request):
    try:
        ...
        # Calculate audit pass rates using raw SQL
        # This query joins audit_findings and compliance tables
        # and groups results by IsRisk (compliance status)
        pass_rate_query = """
            SELECT
                c.IsRisk,
                COUNT(*) AS count,
                ROUND(COUNT(*) * 100.0 / 
                    (SELECT COUNT(*) FROM audit_findings af2 
                     JOIN compliance c2 ON af2.ComplianceId = c2.ComplianceId
                     WHERE af2.AssignedDate BETWEEN %s AND %s), 2) AS percentage
            FROM 
                audit_findings af
            JOIN 
        ...
        # Get framework-specific pass rates
        framework_pass_rates_query = """
            SELECT
                f.FrameworkName,
                SUM(CASE WHEN c.IsRisk = 0 THEN 1 ELSE 0 END) AS compliant_count,
                COUNT(*) AS total_count,
                ROUND(SUM(CASE WHEN c.IsRisk = 0 THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) AS pass_rate
            FROM 
                audit_findings af
            JOIN 
                compliance c ON af.ComplianceId = c.ComplianceId
        ...
        with connection.cursor() as cursor:
            # Get pass rate data
            cursor.execute(pass_rate_query, [start_date, end_date, start_date, end_date])
            pass_rate_data = []
            ...
            overall_pass_rate = 0
            if total_count > 0:
                overall_pass_rate = round((compliant_count / total_count) * 100, 2)
            
            # Get framework-specific pass rates
            cursor.execute(framework_pass_rates_query, [start_date, end_date])
            framework_data = []
            
            for row in cursor.fetchall():
                framework, compliant, total, pass_rate = row
            ...
            for row in cursor.fetchall():
                month_str, pass_rate = row
                if month_str and pass_rate:
                    year, month = map(int, month_str.split('-'))
                    month_date = datetime(year, month, 1)
        ...
        logger.error(f"Error in get_audit_pass_rate: {str(e)}")
        return Response({
            'success': False,
            'message': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_non_compliance_issues(request):
    try:
        ...
        period = request.GET.get('period', 'year')
        year = request.GET.get('year', datetime.now().year)
        severity = request.GET.get('severity', 'all')  # all, major, minor
        ...
        severity_filter = ""
        if severity == 'major':
            severity_filter = "AND af.MajorMinor = '1'"
        elif severity == 'minor':
            severity_filter = "AND af.MajorMinor = '0'"
        ...
        severity_breakdown_query = """
            SELECT 
                CASE
                    ...
                    WHEN af.MajorMinor IS NULL OR af.MajorMinor = '' OR af.MajorMinor = 'NA' THEN 'Unspecified'
                    ELSE af.MajorMinor
                END as severity,
                COUNT(*) as count
            FROM 
        ...
        with connection.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                # Map the severity values to their proper labels
                if severity == '1':
...
def get_severity_distribution(request):
    try:
        ...
        severity_query = """
            SELECT 
                CASE
                    ...
                    WHEN MajorMinor IS NULL OR MajorMinor = '' OR MajorMinor = 'NA' THEN 'Unspecified'
                    ELSE MajorMinor
            ...
            GROUP BY 
                severity_level
            ORDER BY 
                FIELD(severity_level, 'Major', 'Minor', 'Unspecified')
        """
        ...
        trend_query = """
            ...
            ORDER BY 
                month, FIELD(severity_level, 'Major', 'Minor', 'Unspecified')
        """
            ...
            for month_str, severities in trend_data.items():
                if month_str:
                    ...
                    month_data = {
                        ...
                        'Unspecified': severities.get('Unspecified', 0)
                    }
