...
def get_latest_version(audit_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("""
                SELECT version_number, version_data
                FROM audit_version 
                WHERE audit_id = %s
                ORDER BY created_at DESC
                LIMIT 1
        ...
        # Handle acceptance flow
            ...
            with connection.cursor() as cursor:
                ...
                if version_row:
                    ...
                    if audit_evidence:
                        print(f"DEBUG: Updating audit Evidence with metadata audit_evidence: {audit_evidence}")
                        cursor.execute("""
                            UPDATE audit
                            SET Evidence = %s
                            WHERE AuditId = %s
                        """, [audit_evidence, audit_id])
                    ...
                    if 'audit_evidence' in existing_metadata and existing_metadata['audit_evidence']:
                        ...
                        print(f"DEBUG: Updating audit Evidence from preserved metadata: {metadata['audit_evidence']}")
                        cursor.execute("""
                            UPDATE audit
                            SET Evidence = %s
                            WHERE AuditId = %s
                        """, [metadata['audit_evidence'], audit_id])
