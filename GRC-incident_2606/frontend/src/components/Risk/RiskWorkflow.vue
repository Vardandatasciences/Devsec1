  ...
  <div class="workflow-container">
    ...
    <div v-if="!showMitigationWorkflow && !showReviewerWorkflow">
      ...
      <div class="user-filter">
        ...
        <div v-if="loading && users.length === 0" class="loading-indicator">Loading users...</div>
        <select id="user-select" v-model="selectedUserId" @change="fetchData" class="user-dropdown" :disabled="loading && users.length === 0">
            ...
            <table class="user-tasks-table">
              ...
              <tbody>
                <tr v-for="risk in userRisks" :key="risk.RiskInstanceId">
                  ...
                  <td>{{ risk.Category }}</td>
                  <td>
                    <span class="criticality-badge" :class="risk.Criticality?.toLowerCase()">{{ risk.Criticality }}</span>
                  </td>
                  <td>{{ risk.RiskDescription }}</td>
                  <td>
                    <span :class="'status-' + (risk.RiskStatus?.toLowerCase().replace(/\s+/g, '-') || 'not-assigned')">
                    ...
                    <span v-if="risk.MitigationStatus" class="mitigation-status" :class="getMitigationStatusClass(risk.MitigationStatus)">
                      {{ risk.MitigationStatus }}
                    </span>
                    <span v-if="risk.MitigationDueDate" class="due-status" :class="getDueStatusClass(risk.MitigationDueDate)">
            ...
            <table class="user-tasks-table">
              ...
              <tbody>
                <tr v-for="task in reviewerTasks" :key="task.RiskInstanceId">
                  ...
                  <td>
                    <span class="criticality-badge" :class="task.Criticality?.toLowerCase() || 'unknown'">{{ task.Criticality || 'Unknown' }}</span>
                  </td>
                  ...
                  <td>
                    ...
                    <span v-if="task.MitigationStatus" class="mitigation-status" :class="getMitigationStatusClass(task.MitigationStatus)">
