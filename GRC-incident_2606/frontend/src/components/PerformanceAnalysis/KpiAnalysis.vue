<template>
  <div class="kpi-container">
    <!-- FIRST ROW: Audit Pass Rate, Severity of Issues -->
    <div class="kpi-row">
      <!-- Audit Pass Rate Card -->
      <div class="kpi-card-wrapper" style="flex: 2; min-width: 300px; max-width: 30%;">
        <v-card 
            ...
            <div class="d-flex flex-column align-center pa-4">
              ...
              <div class="period-selector">
                <button 
                  ...
                  :key="p"
                  class="period-button"
                  :class="{ active: passRatePeriod === p }"
                  @click="changePassRatePeriod(p)"
                >
              ...
              <div class="gauge-container">
                <v-progress-circular
                  :model-value="passRateMetrics.overall_pass_rate || 0"
                  :size="130"
                  :width="15"
                  :color="getPassRateColor"
                  class="gauge-chart"
                >
                  {{ passRateMetrics.overall_pass_rate || 0 }}%
                </v-progress-circular>
              </div>
              
              <!-- Title -->
              <div class="text-h6 mt-3 text-center">Audit Pass Rate</div>
              
              <!-- Rating info -->
              <div class="target-info">
                <div class="target-label">{{ passRateMetrics.compliant_count || 0 }} of {{ passRateMetrics.total_count || 0 }} findings compliant</div>
                <div class="efficiency-badge" :class="getPassRateClass">
                  {{ passRateMetrics.rating || 'N/A' }}
                </div>
              </div>
              
              <!-- Framework info if available -->
              <div class="text-body-2 mt-1 text-center" v-if="passRateMetrics.frameworks && passRateMetrics.frameworks.length">
                Best: {{ passRateMetrics.frameworks[0].framework }} ({{ passRateMetrics.frameworks[0].pass_rate }}%)
              </div>
            </div>
          </v-card-item>

          <!-- Error message -->
          <v-card-text v-if="passRateError" class="error-text">
      ...
      <div class="kpi-card-wrapper" style="flex: 5; min-width: 500px;">
        <v-card 
            ...
            <div class="d-flex flex-column align-center pa-4">
              ...
              <div class="time-to-close-container">
                  ...
                  <div v-if="timeToCloseMonthlyTrend.length > 0" class="close-time-chart-container">
                    ...
                    <div class="line-chart close-time-chart">
                      ...
                      >
                        ...
                        <div class="tooltip-rate">{{ closeTimeTooltipData.avg_close_days }} days</div>
                ...
                <div class="right-section">
                  <!-- Top Oldest Open Findings Table -->
                  <div class="oldest-findings-section">
                    <div class="section-title">Top Oldest Open Findings</div>
                    
                    <div v-if="timeToCloseMetrics.oldest_findings && timeToCloseMetrics.oldest_findings.length" class="oldest-findings-table">
                      <table class="findings-table">
                        ...
                        <tbody>
                          <tr v-for="(finding, index) in timeToCloseMetrics.oldest_findings.slice(0, 5)" :key="index" 
                              :class="{'critical-age': finding.days_open > timeToCloseMetrics.target_days * 2, 
                                      'warning-age': finding.days_open > timeToCloseMetrics.target_days && finding.days_open <= timeToCloseMetrics.target_days * 2}">
                            <td class="days-cell">{{ finding.days_open }}</td>
                            <td>{{ finding.audit_id }}</td>
                    ...
                    <div v-else class="no-data-message">
                      No open findings data available
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="d-flex flex-column align-center pa-4">
              ...
              <div class="severity-breakdown">
                <div class="severity-item">
                  <div class="severity-name">NA:</div>
                  <div class="severity-count">12</div>
                </div>
                <div class="severity-item">
                  <div class="severity-name">NA:</div>
                  <div class="severity-count">44</div>
                </div>
                <div class="severity-item">
                  <div class="severity-name">NA:</div>
              ...
              <div class="text-body-2 mt-1 text-center">
                Top area: Password Complexity Requirements (19 issues)
              </div>
    ...
    <div class="kpi-row">
      ...
      <div class="kpi-card-wrapper">
        ...
        >
            ...
            <div class="d-flex flex-column align-center pa-4">
              ...
              <div class="period-selector">
                <button 
                  ...
                  :key="p"
                  class="period-button"
                  :class="{ active: closureRatePeriod === p }"
                  @click="changeClosureRatePeriod(p)"
                >
                  {{ p.charAt(0).toUpperCase() + p.slice(1) }}
                </button>
              </div>

              <!-- Circular Progress for Closure Rate -->
              <div class="gauge-container">
                <v-progress-circular
                  :model-value="closureRateMetrics.closure_rate || 0"
                  :size="130"
                  :width="15"
                  :color="getClosureRateColor"
                  class="gauge-chart"
                >
                  {{ closureRateMetrics.closure_rate || 0 }}%
                </v-progress-circular>
              </div>
              
              <!-- Title -->
              <div class="text-h6 mt-3 text-center">Findings Closure Rate</div>
              
              <!-- Count info -->
              <div class="target-info">
                <div class="target-label">{{ closureRateMetrics.closed_count || 0 }} of {{ closureRateMetrics.opened_count || 0 }} findings closed</div>
                <div class="efficiency-badge" :class="getClosureRateClass">
                  {{ closureRateMetrics.rating || 'N/A' }}
                </div>
              </div>
              
              <!-- Average time to close -->
              <div class="text-body-2 mt-1 text-center">
                Avg. {{ closureRateMetrics.avg_days_to_close || 0 }} days to close
              </div>
              
              <!-- Line chart for trend -->
              <div class="trend-chart-container" v-if="closureRateTrend.length > 0">
                <div class="chart-title">Closure Rate Trend</div>
                <div class="line-chart">
                  <!-- Background grid lines -->
                  <div class="chart-grid">
                  ...
                  <!-- Y-axis labels -->
                  <div class="chart-y-axis">
                    <div class="chart-y-label" style="bottom: 0%">0%</div>
                    <div class="chart-y-label" style="bottom: 20%">20%</div>
                    <div class="chart-y-label" style="bottom: 40%">40%</div>
                    <div class="chart-y-label" style="bottom: 60%">60%</div>
                    <div class="chart-y-label" style="bottom: 80%">80%</div>
                    <div class="chart-y-label" style="bottom: 100%">100%</div>
                  </div>
                  
                  <!-- The data points -->
                  <div 
                    v-for="(point, index) in closureRateTrend" 
                    :key="index"
                    class="chart-point"
                    :style="{ 
                      left: `${(index / (closureRateTrend.length - 1)) * 100}%`,
                      bottom: `${(point.rate / 100) * 80}%`
                    }"
                    :title="`${point.month}: ${point.rate}% (${point.closed_count}/${point.opened_count})`"
                  ...
                  <!-- The connecting lines -->
                  <div
                    v-for="(point, index) in closureRateTrend.slice(0, -1)" 
                    :key="`line-${index}`"
                    class="chart-line"
                    :style="getChartLineStyle(index)"
                  ></div>
                  ...
                  >
                    ...
                    <div class="tooltip-rate">{{ tooltipData.rate }}%</div>
                    <div class="tooltip-counts">{{ tooltipData.closed_count }} of {{ tooltipData.opened_count }} closed</div>
                  </div>
                </div>
          ...
          <!-- Error message -->
          <v-card-text v-if="closureRateError" class="error-text">
            <v-icon color="error" class="mr-2">mdi-alert-circle</v-icon>
            {{ closureRateError }}
          </v-card-text>
        </v-card>
...
// Audit Pass Rate state
const passRateLoading = ref(false);
const passRateError = ref(null);
const passRatePeriod = ref('year');
const passRateMetrics = ref({
  overall_pass_rate: 0,
  compliant_count: 0,
  total_count: 0,
  rating: 'N/A',
...
// Closure Rate state
const closureRateLoading = ref(false);
const closureRateError = ref(null);
const closureRatePeriod = ref('year');
const closureRateMetrics = ref({
  closure_rate: 0,
  closed_count: 0,
  opened_count: 0,
  avg_days_to_close: 0,
...
const getPassRateColor = computed(() => {
  const rate = passRateMetrics.value.overall_pass_rate || 0;
...
const getPassRateClass = computed(() => {
  const rating = passRateMetrics.value.rating || '';
  
  if (rating === 'Excellent') return 'pass-rate-excellent';
  if (rating === 'Good') return 'pass-rate-good';
  if (rating === 'Fair') return 'pass-rate-fair';
...
const getClosureRateColor = computed(() => {
  return closureRateMetrics.value.color || 'info';
});

const getClosureRateClass = computed(() => {
  const rating = closureRateMetrics.value.rating || '';
  
  if (rating === 'Excellent') return 'closure-rate-excellent';
  if (rating === 'Good') return 'closure-rate-good';
  if (rating === 'Fair') return 'closure-rate-fair';
  return 'closure-rate-poor';
});
...
const fetchTimeToCloseMetrics = async () => {
  ...
  try {
    const response = await axios.get(`http://localhost:8000/api/kpi/time-to-close/?period=${timeToClosePeriod.value}`);
    if (response.data.success && response.data.data) {
      timeToCloseMetrics.value = response.data.data.metrics;
      timeToCloseMetrics.value.oldest_findings = response.data.data.oldest_findings || [];
      
      // Process monthly trend data for chart
      timeToCloseMonthlyTrend.value = response.data.data.monthly_trend || [];
      ...
      throw new Error(response.data.message || 'Failed to fetch time to close metrics');
    }
  } catch (err) {
...
const fetchPassRateMetrics = async () => {
  passRateLoading.value = true;
  passRateError.value = null;
  
  try {
    const response = await axios.get(`http://localhost:8000/api/kpi/audit-pass-rate/?period=${passRatePeriod.value}`);
    if (response.data.success && response.data.data) {
      passRateMetrics.value = response.data.data.metrics;
      passRateMetrics.value.distribution = response.data.data.distribution || [];
      passRateMetrics.value.frameworks = response.data.data.frameworks || [];
      passRateMetrics.value.monthly_trend = response.data.data.monthly_trend || [];
    } else {
      throw new Error(response.data.message || 'Failed to fetch pass rate metrics');
    }
  } catch (err) {
    console.error('Error fetching pass rate metrics:', err);
    passRateError.value = 'Failed to load pass rate data';
  } finally {
...
const fetchClosureRateMetrics = async () => {
  closureRateLoading.value = true;
  closureRateError.value = null;
  
  try {
    const response = await axios.get(`http://localhost:8000/api/kpi/closure-rate/?period=${closureRatePeriod.value}`);
    if (response.data.success && response.data.data) {
      closureRateMetrics.value = response.data.data.metrics;
      
      // Process trend data for chart
      const trendData = response.data.data.monthly_trend || [];
      closureRateTrend.value = trendData.map(item => ({
        month: item.month,
      ...
      throw new Error(response.data.message || 'Failed to fetch closure rate metrics');
    }
  } catch (err) {
    console.error('Error fetching closure rate metrics:', err);
    closureRateError.value = 'Failed to load closure rate data';
  } finally {
    closureRateLoading.value = false;
  }
};
...
const changePassRatePeriod = async (newPeriod) => {
  passRatePeriod.value = newPeriod;
  await fetchPassRateMetrics();
};
...
const changeClosureRatePeriod = async (newPeriod) => {
  closureRatePeriod.value = newPeriod;
  await fetchClosureRateMetrics();
};
...
const getChartLineStyle = (index) => {
  const startPoint = closureRateTrend.value[index];
  const endPoint = closureRateTrend.value[index + 1];
  
  if (!startPoint || !endPoint) return {};
  
  const x1 = (index / (closureRateTrend.value.length - 1)) * 100;
  const y1 = (startPoint.rate / 100) * 80;
  const x2 = ((index + 1) / (closureRateTrend.value.length - 1)) * 100;
  const y2 = (endPoint.rate / 100) * 80;
...
const showTooltip = (index, event) => {
  const point = closureRateTrend.value[index];
  if (!point) return;
...
const getCloseTimeLineStyle = (index) => {
  ...
  // Ensure line is contained within chart bounds
  return {
    width: `${length}%`,
    left: `${x1}%`,
    bottom: `${y1}%`,
...
onMounted(() => {
  ...
  fetchPassRateMetrics();
  fetchIssuesMetrics();
  fetchSeverityMetrics();
  fetchClosureRateMetrics();
