        ...
        <li v-for="approval in complianceApprovals" :key="approval.ApprovalId">
          ...
          <div class="approval-details">
            ...
            <div class="meta-info">
              ...
              <span v-if="approval.ApprovalDueDate" class="due-date" :class="getDueDateClassFor(approval)">
                <i class="fas fa-clock"></i>
                {{ getDueDateTextFor(approval) }}
              </span>
            </div>
          </div>
    ...
    <div v-if="showDetails && selectedApproval" class="policy-details-modal">
      <div class="policy-details-content">
        ...
        <div class="policy-approval-section">
          ...
          <!-- Add due date information in details view -->
          <div v-if="selectedApproval.ApprovalDueDate" class="due-date-details" :class="getDueDateClassFor(selectedApproval)">
            <div class="due-date-label">
              <i class="fas fa-calendar-check"></i>
              Due Date:
            </div>
            <div class="due-date-value">
              {{ formatDate(selectedApproval.ApprovalDueDate) }}
              <span class="due-date-status">{{ getDueDateTextFor(selectedApproval) }}</span>
            </div>
          </div>
        ...
        <div v-if="selectedApproval.ExtractedData" class="compliance-details">
          ...
          <div class="compliance-detail-row">
            ...
            <div class="mitigation-steps">
              <div v-if="formatMitigationSteps(selectedApproval.ExtractedData.mitigation).length > 0" 
                   class="steps-container">
                <div v-for="(step, index) in formatMitigationSteps(selectedApproval.ExtractedData.mitigation)" 
                     :key="index" 
              ...
              <div v-else class="no-mitigation">
  ...
  methods: {
    async refreshData() {
      ...
      try {
        ...
        PopupService.error('Failed to load approval data. Please try refreshing the page.', 'Loading Error');
      } finally {
    ...
    async submitReview() {
      const approvalType = this.selectedApproval.ExtractedData?.type === 'compliance_deactivation' || 
                          this.selectedApproval.ExtractedData?.RequestType === 'Change Status to Inactive' 
                          ? 'deactivation request' : 'compliance item';
      
      PopupService.confirm(
        `Do you want to approve this ${approvalType}?`,
        'Review Confirmation',
    ...
    showRejectPopup() {
      ...
      PopupService.comment(
        `Please provide a reason for rejecting this ${approvalType}:`,
        'Reject Item',
      ...
      try {
        ...
        if (this.selectedApproval.ExtractedData?.type === 'compliance_deactivation' || 
          ...
          PopupService.success(
            isApproved ? 'Deactivation request has been approved successfully!' : 'Deactivation request has been rejected.',
            isApproved ? 'Deactivation Approved' : 'Deactivation Rejected'
          );
        } else {
          PopupService.success(
            isApproved ? 'Compliance has been approved successfully!' : 'Compliance has been rejected.',
            isApproved ? 'Approval Complete' : 'Rejection Complete'
          );
        }
        ...
        // Show error popup
        PopupService.error(`Error submitting review: ${error.message || 'Unknown error'}`, 'Review Error');
      } finally {
    ...
    // Define utility functions for due date handling
    getDueDateClassFor(approval) {
      if (!approval || !approval.ApprovalDueDate) return '';
      
      const daysRemaining = this.calculateDaysRemaining(approval.ApprovalDueDate);
      
      if (daysRemaining < 0) {
        return 'overdue'; // Past due date
      } else if (daysRemaining <= 2) {
        return 'due-soon'; // Due within 2 days
      } else {
        return 'on-track'; // Due date is comfortably in the future
      }
    },
    ...
    calculateDaysRemaining(dueDateStr) {
      ...
      dueDate.setHours(0, 0, 0, 0); // Reset time part for accurate day calculation
    ...
    formatMitigationSteps(mitigationData) {
      if (!mitigationData) return [];
      
      try {
        // If it's already an array, return it
        if (Array.isArray(mitigationData)) {
          return mitigationData.filter(step => step && step.trim() !== '');
        }
        
        // If it's an object, try to extract steps
        if (typeof mitigationData === 'object') {
          // Check for common step properties
          if (mitigationData.steps && Array.isArray(mitigationData.steps)) {
            return mitigationData.steps.filter(step => step && step.trim() !== '');
          }
          
          // Convert object values to steps
          const steps = Object.values(mitigationData)
        ...
        if (typeof mitigationData === 'string') {
          try {
            const parsed = JSON.parse(mitigationData);
            return this.formatMitigationSteps(parsed); // Recursive call with parsed data
          } catch (e) {
                         // If JSON parsing fails, treat as plain text and split by common delimiters
             const steps = mitigationData
               .split(/[\n\r]+|(?:\d+[.)]\s*)|(?:-\s*)|(?:\*\s*)/) // Split by newlines, numbered items, dashes, or bullets
               .map(step => step.trim())
               .filter(step => step !== '' && step.length > 3); // Filter out empty or very short strings
            
            return steps.length > 0 ? steps : [mitigationData]; // Return original if no meaningful splits
          }
        }
        
        return [];
      } catch (error) {
        console.error('Error formatting mitigation steps:', error);
        // Return original data as single step if all parsing fails
        return typeof mitigationData === 'string' ? [mitigationData] : [];
      }
    }
  },
  computed: {
