      ...
      <table class="audits-table">
        ...
        <tbody>
          <tr v-for="(audit, idx) in audits" :key="audit.audit_id">
            ...
            <td>
              <button 
                v-if="audit.status === 'Yet to Start'"
                class="auditor-card-status status-yet"
                @click="startAudit(idx)"
              >
                Start
              </button>
              <button 
                v-else-if="audit.status === 'Work In Progress'"
                class="auditor-card-status status-progress"
                @click="openPopup(idx)"
              >
                Edit Audit
              </button>
              <button 
                v-else
                class="auditor-card-status"
                :class="{
                  'status-review': audit.status === 'Under review',
                  'status-completed': audit.status === 'Completed'
                }"
                @click="openPopup(idx)"
                :disabled="audit.status === 'Completed' || audit.status === 'Under review'"
                :title="audit.status === 'Under review' ? 'This audit is currently under review and cannot be modified' : 
                       audit.status === 'Completed' ? 'This audit has been completed' : ''"
              >
                {{ audit.status }}
              </button>
            </td>
            <td>
              ...
              >
                <i class="fas fa-file-download"></i> View Previous Reports
              </button>
              <button 
                v-if="audit.status === 'Completed'"
                class="report-btn view-versions-btn" 
                @click="viewAuditVersions(audit)"
                title="View all versions of this audit"
              >
      ...
      <div v-for="(audit, idx) in audits" :key="audit.audit_id" class="audit-card">
        ...
        <div class="card-body">
          <div class="card-info-row">
            <span class="card-info-icon"><i class="fas fa-hashtag"></i></span>
            <span class="card-info-label">Audit ID:</span>
            <span class="card-info-value">{{ audit.audit_id }}</span>
          </div>
          <div class="card-info-row">
