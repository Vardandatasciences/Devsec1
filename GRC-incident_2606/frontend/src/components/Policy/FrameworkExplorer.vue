...
const toggleStatus = async (fw) => {
  try {
    ...
    if (fw.status === 'Active') {
      ...
      const reason = prompt('Please provide a reason for deactivating this framework:', 'No longer needed');
      ...
      await axios.post(`/api/frameworks/${fw.id}/toggle-status/`, {
        reason,
        cascadeToApproved: true
      });
      
      // Show success message
      alert('Framework deactivation request submitted. Awaiting approval.');
