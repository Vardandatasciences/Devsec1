  ...
  <div class="dashboard-container">
    ...
    <div v-else>
      ...
      <div v-if="pendingRequests.length > 0" class="approvals-list">
        <ul>
          <li v-for="request in pendingRequests" 
            ...
            <span class="item-type-badge request-badge">STATUS CHANGE</span>
            <span class="date-info">
              {{ formatDate(request.RequestDate) }}
            </span>
            <span class="framework-category">{{ request.Category }}</span>
      ...
      <div class="framework-grid">
        <div v-for="framework in requests" :key="framework.ApprovalId" class="framework-card">
          <div class="framework-header">
            <i class="fas fa-book"></i>
            <span>{{ framework.FrameworkName }}</span>
          </div>
          
          <div class="category-tag">Category: {{ framework.Category }}</div>
          
          <div class="framework-description">{{ framework.Reason }}</div>
          
          <div class="framework-footer">
            <div class="status-toggle">
              <input 
                type="checkbox" 
                :checked="framework.Status === 'Approved'" 
                disabled
              />
              <span class="switch-label" :class="{
                'active': framework.Status === 'Rejected' || framework.Status === 'Pending Approval',
                'inactive': framework.Status === 'Approved'
              }">
                {{ framework.Status === 'Approved' ? 'Inactive' : 
                   framework.Status === 'Rejected' ? 'Active' : 'Pending' }}
              </span>
            </div>
            
            <div class="actions">
              <button class="view-btn" @click="openRequestDetails(framework)">
    ...
    <div v-if="showDetails && selectedRequest" class="framework-details-modal">
      <div class="framework-details-content">
        <h3>
          <span class="detail-type-indicator">STATUS CHANGE REQUEST</span> 
          Details: {{ selectedRequest.FrameworkName }}
        ...
        <div class="request-details">
          <div class="framework-detail-row">
            <strong>Framework Name:</strong> <span>{{ selectedRequest.FrameworkName }}</span>
          </div>
          <div class="framework-detail-row">
            <strong>Category:</strong> <span>{{ selectedRequest.Category }}</span>
          </div>
          <div class="framework-detail-row">
        ...
        <div v-if="selectedRequest.AffectedPolicies && selectedRequest.AffectedPolicies.length > 0" class="affected-policies-section">
          <h4>Affected Policies</h4>
          <p class="section-description">
            <span v-if="selectedRequest.Status === 'Approved'">
              The following policies have been set to Inactive:
            </span>
            <span v-else>
              The following policies will become inactive if this request is approved:
            </span>
          </p>
          
          <div class="policies-list">
            <div v-for="policy in selectedRequest.AffectedPolicies" :key="policy.PolicyId" class="policy-item">
...
const fetchRequests = async () => {
  ...
  try {
    const response = await axios.get('/api/framework-status-change-requests/')
    
    // Process all frameworks to ensure consistent status
    requests.value = processFrameworks(response.data)
...
const approveRequest = async (request) => {
  const confirmed = confirm(`Are you sure you want to approve changing the status of "${request.FrameworkName}" to Inactive?${request.CascadeToApproved ? ' This will also make all approved policies inactive.' : ''}`)
  ...
  try {
    ...
    await axios.post(`/api/framework-approvals/${request.ApprovalId}/approve-status-change/`, {
      approved: true,
      remarks: remarks || 'Status change approved'
    })
    
    alert(`Framework "${request.FrameworkName}" has been set to Inactive.${request.PolicyCount > 0 ? ` ${request.PolicyCount} policies were also made inactive.` : ''}`)
    ...
    alert('Failed to approve status change request. Please try again.')
  }
}

// Reject status change request
const rejectRequest = async (request) => {
  const confirmed = confirm(`Are you sure you want to reject the status change request for "${request.FrameworkName}"? The framework will remain Active.`)
  ...
  try {
    ...
    alert(`Status change request for "${request.FrameworkName}" has been rejected. The framework remains Active.`)
    ...
    alert('Failed to reject status change request. Please try again.')
  }
}
