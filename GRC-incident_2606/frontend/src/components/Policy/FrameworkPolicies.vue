...
const exportPolicy = async (policyId) => {
  ...
  try {
    ...
    if (!file_url || !file_name) {
      alert('Export failed: No file URL or name returned.');
      return;
    }
    try {
      ...
      alert('Export failed during file download.');
...
const toggleStatus = async (policy) => {
  try {
    // Direct toggle without approval (keep existing behavior)
    ...
    if (response.data.subpolicies_affected > 0) {
      message += ` ${response.data.subpolicies_affected} subpolicies were also ${response.data.status === 'Active' ? 'activated' : 'deactivated'}.`;
    }
