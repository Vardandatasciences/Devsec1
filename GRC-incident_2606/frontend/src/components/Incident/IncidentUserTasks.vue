                ...
                <tbody>
                  <tr v-for="incident in userIncidents" :key="incident.id">
                    ...
                    <td>
                      ...
                      <div v-if="incident.Status === 'Rejected'" class="rejection-details">
                        <div class="rejection-notice">
                          <small>{{ incident.RejectionSource ? 
                            `Rejected from ${incident.RejectionSource === 'AUDIT_FINDING_REVIEW' ? 'Audit Review' : incident.RejectionSource}` : 
                            'Rejected by Reviewer' }}</small>
                        </div>
                        <div class="resubmission-indicator">
                          <small><strong>⚠️ Needs Resubmission</strong></small>
                        </div>
                      </div>
                      <span v-if="incident.MitigationDueDate" class="due-status" 
                            :class="[getDueStatusClass(incident.MitigationDueDate), incident.Status === 'Rejected' && isOverdue(incident.MitigationDueDate) ? 'rejected-overdue' : '']">
                        {{ getDueStatusText(incident.MitigationDueDate) }}
                      </span>
                    </td>
                    <td>{{ formatDate(incident.MitigationDueDate) }}</td>
                    <td>
                      <button @click="viewMitigations(incident.id)" 
                              :class="['view-btn', 
                                       incident.Status === 'Rejected' ? 'rejected-incident' : '',
                                       incident.Status === 'Approved' ? 'approved-incident' : '']"
                              :title="incident.Status === 'Rejected' ? 'Click to resubmit this rejected incident' : 
                                     incident.Status === 'Approved' ? 'View completed and approved incident' : 
                                     'View incident details'">
                        <i :class="incident.Status === 'Rejected' ? 'fas fa-redo' : 
                                  incident.Status === 'Approved' ? 'fas fa-check-circle' : 
                                  'fas fa-eye'"></i> 
                        {{ incident.Status === 'Rejected' ? 'Resubmit' : 
                           incident.Status === 'Approved' ? 'Completed' : 
                           'View' }}
                      </button>
                    </td>
                  </tr>
                ...
                <tbody>
                  <tr v-for="task in reviewerTasks" :key="task.id">
                    ...
                    <td>
                      ...
                      <div v-if="getUserName(task.AssignerId)" style="font-size:12px;color:#888;">By: {{ getUserName(task.AssignerId) }}</div>
                    </td>
                    <td>{{ getUserName(task.AssignerId) }}</td>
                    <td>
                      <button 
                        @click="reviewMitigations(task)" 
                        :class="['review-btn', { 'approved': task.Status === 'Approved' }]"
                        :title="task.Status === 'Approved' ? 'View approved incident' : 'Review incident'"
                      >
                        <i :class="task.Status === 'Approved' ? 'fas fa-eye' : 'fas fa-tasks'"></i> 
                        {{ task.Status === 'Approved' ? 'View' : 'Review' }}
                      </button>
                    </td>
                  </tr>
                </tbody>
  ...
  export default {
    ...
    methods: {
      ...
      handleFileUpload(event, index) {
        ...
        reader.onload = (e) => {
          axios.post('http://localhost:8000/api/upload-file/', {
