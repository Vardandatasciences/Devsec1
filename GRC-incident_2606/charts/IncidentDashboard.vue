  ...
  <div class="incident-dashboard-wrapper">
    ...
    <div class="kpi-row">
      ...
      <div class="kpi-card">
        ...
        <div class="kpi-value">
          {{ kpiData.mttr.value }}<span class="unit">{{ kpiData.mttr.unit }}</span>
    ...
    <div class="kpi-row">
      ...
      <div class="kpi-card compact-card">
      ...
      try {
        ...
        try {
          ...
          if (mttrResponse.data) {
            const rawValue = mttrResponse.data.value;
            const parsedValue = parseFloat(rawValue);
            
            // Process trend data to ensure it's properly formatted for the chart
            const trendData = mttrResponse.data.trend.map(item => {
              return {
                month: item.month,
                minutes: parseFloat(item.minutes || 0),
                count: item.count || 0,
            ...
            console.log("Processed MTTR trend data:", trendData);
            
            // Update MTTR data
            this.kpiData.mttr = {
              value: parsedValue,
              unit: mttrResponse.data.unit,
              trend: trendData,
  ...
  console.error('Error fetching cost per incident:', error);
    ...
    generateTrendPath(dataPoints) {
      if (!dataPoints || dataPoints.length === 0) {
        console.log("No data points for trend path");
        return '';
      }
      
      console.log("Generating trend path from points:", dataPoints);
    ...
    // Fixed method to get points for trend circles
    getTrendPoints(dataPoints) {
      ...
      console.log("Generated trend points:", points);
      return points;
    },
    fetchIncidentCounts() {
